...
2. **Configure environment variables** — copy `.env.local.example` to `.env.local` and fill in real values:
   ...
   | `DEMO_MODE` | `"true"`/`"false"`, default `false`. When `true`, allowlisted emails sign in directly without email verification — **for evaluation only**, see [Security model](#security-model) |
...
- **`DEMO_MODE` is an intentional auth bypass — evaluation only.** With `DEMO_MODE=true`, `POST /api/auth/request-link` still enforces the allowlist (unregistered emails still 403) but skips proving inbox ownership: it mints a Supabase magic-link token server-side via `supabaseAdmin.auth.admin.generateLink` and hands the token hash straight to the browser, which verifies it immediately. This means anyone who knows an allowlisted email address can sign in as that person with no access to their inbox. Leave it unset (or `false`) for any deployment where that matters.
